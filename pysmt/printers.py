...
class SmartPrinter(HRPrinter):
    ...
    'smarties' is found, this is replaced.

    E.g., smarties = {And(a,b): "ab"}
    ...
    def __init__(self, stream, smarties=None):
    ...
    def smart_walk(self, formula):
        if formula not in self.smarties:
            ...
            self.write(self.smarties[formula])
            return True
...
def smart_serialize(formula, smarties=None, threshold=None):
    ...
    buf = cStringIO()
    p = SmartPrinter(buf, smarties=smarties)
